<template>
  <div class="bg-black">
    ...
    <main class="xl:px-[80px] md:px-[50px] px-[20px]">
      <h1 class="text-center mt-11 text-5xl font-bold text-blue-800 leading-tight">
      ...
      <div class="justify-center mt-11 block md:hidden">
        <select class="appearance-none bg-red-400 text-white outline-none border-none p-3 w-[300px] text-center" name=""
          id="">
      ...
      <div class="my-20 grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-12">
        ...
        <template v-if="!gettingRecipes">
          <router-link :to="{ name: 'detail', params: { id: recipe.id } }" v-for="recipe in recipes" :key="recipe.id">
            <div
              ...
              <img class="h-[300px] mb-4 rounded-t-lg w-full object-cover" :src="'http://localhost:8000' + recipe.photo"
                alt="product image" />
              <div class="px-5 pb-5">
        ...
        <div class="flex gap-[30px]">
          ...
          <template v-for="link in links" :key="link.label">
            <button class="text-red-600" @click="getPaginatedRecipe(prev_page_url)" v-if="link.label.includes('Previous')">{{ '<' }}
                Previous</button>
                <button class="text-red-600" @click="getPaginatedRecipe(next_page_url)" v-else-if="link.label.includes('Next')">Next
                  {{ '>' }}</button>
                <button v-else v-html="link.label"
                  :class="link.label == current_page ? 'bg-red-400 text-white' : 'text-red-600'"
                  style="padding: 10px 20px;" @click="getPaginatedRecipe(link?.url)" />
          </template>
        </div>
      </div>
    </main>
